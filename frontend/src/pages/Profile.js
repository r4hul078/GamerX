...
function Profile() {
  ...
  return (
    <div className={`profile-modern-fullscreen ${theme === 'dark' ? 'dark' : 'light'}`}> 
        ...
        <div className="profile-modern-main">
          <div className="profile-modern-sidebar">
            <div className="profile-modern-avatar">
              <img src={avatarPreview || '/assets/default-avatar.svg'} alt="avatar" />
              <div className="profile-modern-username">{user?.username || 'User'}</div>
              <div className="profile-modern-role">{user?.role === 'admin' ? 'Administrator' : 'Premium User'}</div>
              <label className="profile-modern-upload">
                <input type="file" accept="image/*" onChange={handleAvatarChange} />
                <span>Change Photo</span>
              </label>
              {avatarFile && (
                <div className="profile-modern-avatar-actions">
                  <button onClick={handleUploadAvatar} className="small-btn">Upload</button>
                  <button onClick={() => { setAvatarPreview(user?.profile_picture || null); setAvatarFile(null); }} className="small-btn">Reset</button>
                </div>
              )}
            </div>
            <div className="profile-modern-info">
              <div><span className="label">Email</span><span className="value">{user?.email || '-'}</span></div>
              <div><span className="label">Role</span><span className="value">{user?.role || '-'}</span></div>
            </div>
          </div>
          <div className="profile-modern-content">
            <div className="profile-modern-header">
              <h1>My Profile</h1>
              <button className="profile-modern-back" onClick={() => navigate(user?.role === 'admin' ? '/admin-dashboard' : '/dashboard')}>← Back</button>
            </div>
            <form className="profile-modern-form" onSubmit={handleReset}>
              <h2>Change Password</h2>
              {error && <div className="error-message">{error}</div>}
              {message && <div className="success-message">{message}</div>}
              <div className="profile-modern-form-row">
            ...
            <div className="profile-modern-orders">
              <div className="profile-modern-orders-header">
                <h2>Order History</h2>
                <span className="profile-modern-orders-count">{orders.length} orders</span>
              </div>
              {ordersLoading ? (
